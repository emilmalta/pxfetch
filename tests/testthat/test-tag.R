...
test_that("px_tag() returns a tbl_px", {
  df  <- tibble::tibble(x = 1L)
  out <- px_tag(df, "TBL")
  expect_s3_class(out, "tbl_px")
})

test_that("px_tag() prepends tbl_px to the existing class vector", {
  df  <- tibble::tibble(x = 1L)
  out <- px_tag(df, "TBL")
  expect_equal(class(out), c("tbl_px", "tbl_df", "tbl", "data.frame"))
})
...
test_that("px_tag() sets px_class attribute to 'ball'", {
  out <- px_tag(tibble::tibble(x = 1L), "TBL")
  expect_equal(attr(out, "px_class"), "tbl_px")
})
...
make_tbl_px <- function(title = "My table", table_id = "TBL") {
  px_tag(tibble::tibble(a = 1L, b = "x"), table_id = table_id, title = title)
}

test_that("tbl_sum.tbl_px includes 'Title' entry when title is present", {
  x   <- make_tbl_px(title = "Population by region")
  out <- pillar::tbl_sum(x)
  expect_true("Title" %in% names(out))
  expect_equal(out[["Title"]], "Population by region")
})

test_that("tbl_sum.tbl_px omits 'Title' when title is NA", {
  x   <- make_tbl_px(title = NA_character_)
  out <- pillar::tbl_sum(x)
  expect_false("Title" %in% names(out))
})

test_that("tbl_sum.tbl_px omits 'Title' when title is empty string", {
  x   <- px_tag(tibble::tibble(a = 1L), "TBL", title = "")
  out <- pillar::tbl_sum(x)
  expect_false("Title" %in% names(out))
})

test_that("tbl_sum.tbl_px retains the parent tibble header entry", {
  x   <- make_tbl_px()
  out <- pillar::tbl_sum(x)
...
test_that("print output includes table title when present", {
  x     <- make_tbl_px(title = "My population table")
